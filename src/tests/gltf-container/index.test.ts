...
test('gltfContainer: avocado model loads correctly', async function (context) {
  ...
  await assertSnapshot('gltfcontainer avocado', Vector3.create(1, 1, 1), Vector3.create(8, 1, 8))
  ...
  // const result: TakeAndCompareSnapshotResponse = (Testing as any).takeAndCompareSnapshot(params)
  // if (!result.wasExist) {
  //   Error('This is the first time the tool is run. The test took the reference snapshots for future testing.')
  // }
